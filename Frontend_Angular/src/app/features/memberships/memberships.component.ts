...
@Component({
  ...
  template: `
    <section class="page">
      ...
      <ng-container *ngIf="!loading && !error">
        <div class="empty" *ngIf="items.length === 0"><p>No membership plans yet. Create your first plan.</p></div>
        <div class="card-grid" *ngIf="items.length > 0">
          <div class="card" *ngFor="let m of items">
            <div class="card-top">
              <strong>{{ m.name }}</strong>
              <span class="badge" [class.active]="m.isActive" [class.inactive]="!m.isActive">{{ m.isActive ? 'Active' : 'Inactive' }}</span>
            </div>
            <div class="price">{{ m.price | currency }}</div>
            <div class="meta">Validity: {{ m.validityDays }} days</div>
            <p class="benefits" *ngIf="m.benefits">{{ m.benefits }}</p>
            <div class="card-actions">
              <button (click)="toggleActive(m)">{{ m.isActive ? 'Deactivate' : 'Activate' }}</button>
              <button (click)="editItem(m)">Edit</button>
              <button class="danger" (click)="deleteItem(m)">Delete</button>
            </div>
          </div>
        </div>
      </ng-container>
      ...
      <div class="drawer" *ngIf="showForm">
        <div class="drawer-panel">
          ...
          <div class="form">
            <input [(ngModel)]="form.name" placeholder="Plan name" required>
            ...
            <textarea [(ngModel)]="form.benefits" placeholder="Benefits (one per line)"></textarea>
  ...
  styles: [`
    ...
    .benefits{font-size:13px;color:#374151;white-space:pre-line}
...
export class MembershipsComponent {
  ...
  loadAll() {
    ...
    this.api.getAll().subscribe({ next: (d) => { this.items = d; this.loading = false; }, error: () => { this.error = 'Failed to load memberships.'; this.loading = false; } });
  }

  openCreate() { this.editingId = ''; this.form = { name: '', price: 0, validityDays: 30, benefits: '' }; this.showForm = true; }

  editItem(m: any) { this.editingId = m.id; this.form = { name: m.name, price: m.price, validityDays: m.validityDays, benefits: m.benefits || '' }; this.showForm = true; }
  ...
  toggleActive(m: any) { this.api.update(m.id, { isActive: !m.isActive }).subscribe({ next: () => this.loadAll() }); }

  deleteItem(m: any) { if (!confirm(`Delete "${m.name}"?`)) return; this.api.remove(m.id).subscribe({ next: () => this.loadAll() }); }
}
